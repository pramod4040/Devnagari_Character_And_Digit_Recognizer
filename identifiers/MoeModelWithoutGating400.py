...
class MoeModelWithoutGating400:
    ...
    def create_efficientnet_b4_expert(self, input_shape, num_classes, name_prefix="expert_b4"):
        input_tensor = Input(shape=input_shape)
        resized_input = layers.Resizing(300, 300)(input_tensor)
        base_model = EfficientNetB3(
            include_top=False,
    ...
    def create_model(self, input_shape, num_classes, num_experts):
        ...
        # Fix: Pool or flatten before Dense
        pooled = layers.GlobalAveragePooling2D()(input_layer)
        gate_outputs = layers.Dense(num_experts, activation='softmax')(pooled)
...
if __name__ == "__main__":
    ...
    # ---------- test------------ ------ ---------------------
    # go through all the test images and pass it to test the model 
    ...
    model1 = MoeModelWithoutGating400(weight_path=weight_path, class_names=get_class_names())
    ...
    # # You can also analyze class-wise performance
    # results_df = pd.read_csv('test_predictions.csv')
    # class_accuracy = results_df.groupby('True Label').apply(
    #     lambda x: (x['True Label'] == x['Predicted Label']).mean()
    # )
    
    # print("\nClass-wise accuracy:")
    # print(class_accuracy.sort_values())
